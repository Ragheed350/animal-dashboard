...
import { CRUDBuilder, ItemType, RootState, Authenticated, FetchPollinationsAsync, DeletePollinationAsync, InsertPollinationAsync, UpdatePollinationAsync, FetchAnimalsAsync, Pollination_Req, Pollination } from '@core';
    ...
    const tmp: ItemType[] = [
        {
            ...
            type: 'foreign-key',
            foreignKeyArr: animals.filter(el => el.gender === '0').map(el => ({ title: en ? el['name:en'] : el['name:ar'], value: el.animal_no }))
        },
        {
            columnType: {
                title: 'Female',
            ...
            type: 'foreign-key',
            foreignKeyArr: animals.filter(el => el.gender === '1').map(el => ({ title: en ? el['name:en'] : el['name:ar'], value: el.animal_no }))
        },
    ]
