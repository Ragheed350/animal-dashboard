...
import { ApiResult, ApiError, ApiResponseError } from './models';
  ...
  private handleError(error: AxiosError<ApiError>): ApiError {
    try {
      if (error.response) {
        ...
        return { errorType: ApiErrorType.CONNECTION, ...error.request };
      } else {
        return { errorType: ApiErrorType.UNKNOWN, ...error };
      }
    } catch {
      return { errorType: ApiErrorType.UNKNOWN, ...error };
    }
